...
@testset "Hanging Node Step Response Validation" begin
    ...
    function test_step_response_accuracy()
        ...
        try
            plt = ControlPlots.plt.figure("Hanging Node Step Response")
            ControlPlots.plt.plot(t_sim[1:length(x_analytical)], x_analytical, 
                                label="Analytical", linewidth=2)
            ControlPlots.plt.plot(t_sim[1:length(x_sim)], x_sim, "--", 
                                label="Simulation", alpha=0.8)
            ControlPlots.plt.xlabel("Time (s)")
            ControlPlots.plt.ylabel("Displacement (m)")
            ControlPlots.plt.title("Hanging Node Step Response: Analytical vs Simulation")
            ControlPlots.plt.legend()
            ControlPlots.plt.grid(true)
            ControlPlots.plt.show(block=false)
            
            # Save figure
            savefig(joinpath(get_data_path(), "hanging_node_step_response.png"))
            println("Step response plot saved to hanging_node_step_response.png")
        catch e
